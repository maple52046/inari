...
| Run Dev Server             | `skills/run-dev-server/SKILL.md`             | When the task needs to start (or restart) the Next.js dev server locally, optionally on a specific port or under a `BASE_PATH` URL prefix, and confirm it became ready. Triggered by `/run-dev-server`.                                                                                                       |
