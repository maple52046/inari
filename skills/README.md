...
| -------------------------- | -------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Build Binary               | `skills/build-binary/SKILL.md`               | When the task needs to build a standalone Inari server executable, with the web bundle embedded, for a given OS and architecture (defaults `linux`/`amd64`). Triggered by `/build-binary`.                                                                                                                    |
| Build Deb                  | `skills/build-deb/SKILL.md`                  | When the task needs a `.deb` that installs the binary, the configuration, and the systemd unit for Debian or Ubuntu. Triggered by `/build-deb`.                                                                                                                                                               |
| Build Image                | `skills/build-image/SKILL.md`                | When the task needs to build, tag, and push the Inari container image to the registry with either `docker` or `nerdctl`. Triggered by `/build-image`.                                                                                                                                                         |
| Deploy to Kubernetes       | `skills/deploy-k8s/SKILL.md`                 | When the task needs to deploy Inari to Kubernetes: apply the manifests, roll the Deployment to a new image tag, restart to pick up config, or edit the `inari-env` Secret. Triggered by `/deploy-k8s`.                                                                                                        |
| Run Dev Server             | `skills/run-dev-server/SKILL.md`             | When the task needs to start (or restart) the local development servers (Rust API and Vite SPA), optionally on a specific port or under a `BASE_PATH` URL prefix, and confirm they became ready. Triggered by `/run-dev-server`.                                                                              |
