...
description: >-
  ...
  with the web bundle embedded. Use only when the user explicitly invokes
  $build-binary. Container images are handled by the build-image skill.
---
...
Run `.agents/skills/build-binary/scripts/build_binary.sh --help` and stop.
...
_why_ it refuses, because the reasons are not arbitrary:
...
```bash
.agents/skills/build-binary/scripts/build_binary.sh                 # linux/amd64
.agents/skills/build-binary/scripts/build_binary.sh linux arm64
.agents/skills/build-binary/scripts/build_binary.sh --out /tmp/rel
```
