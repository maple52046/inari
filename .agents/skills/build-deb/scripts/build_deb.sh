...
#
# The binary comes from `.agents/skills/build-binary`, so the .deb and the
# container image are assembled from the same artefact and cannot drift. Because that
...
if [ "$SKIP_BINARY" -eq 0 ]; then
  ...
  .agents/skills/build-binary/scripts/build_binary.sh linux "$ARCH" --out "$OUT_DIR"
  echo
fi
