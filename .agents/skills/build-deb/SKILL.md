...
description: >-
  ...
  and the systemd unit. Use only when the user explicitly invokes $build-deb.
  Container images are handled by build-image; a bare binary by build-binary.
---
...
```bash
.agents/skills/build-deb/scripts/build_deb.sh                 # host architecture
.agents/skills/build-deb/scripts/build_deb.sh arm64
.agents/skills/build-deb/scripts/build_deb.sh --skip-binary   # reuse dist/
```
...
refuse to _configure_ where systemd is absent, and a failed configure skips the
