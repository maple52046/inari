...
description: >-
  ...
  docker or nerdctl. Use only when the user explicitly invokes $build-image.
---
...
```bash
.agents/skills/build-image/scripts/build_push.sh [options]
```
...
.agents/skills/build-image/scripts/build_push.sh

# Force nerdctl, build only (no push)
.agents/skills/build-image/scripts/build_push.sh --engine nerdctl --no-push

# Pin a tag and also push :latest
.agents/skills/build-image/scripts/build_push.sh --tag 20260725-110000 --latest
```
