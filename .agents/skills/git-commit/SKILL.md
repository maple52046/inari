...
description: >-
  ...
  git commit (optionally git add and git push). Use when the user invokes
...
- Do not claim in the message that tests were run or add markers that were not
  requested, except for an agent's optional self-attribution trailer described
  below.

#### Agent attribution trailers

- An agent that materially contributed to the commit may append exactly one
  trailer for itself without asking separately:
  `Co-authored-by: Agent Name <agent@example.com>`.
- The contributing agent chooses whether to add its own trailer and chooses an
  agent-specific display name and syntactically valid email address. The
  identity must clearly represent the agent and must not impersonate a person.
- Do not create a trailer for the user, another person, or another agent. Each
  agent may provide only its own attribution.
- If the user explicitly requests or forbids attribution, follow that
  instruction instead of the default freedom above.
- Place agent attribution after all other footers so it remains at the absolute
  end of the commit message. Separate the footer block from the body with one
  blank line; do not insert blank lines between consecutive trailers.
